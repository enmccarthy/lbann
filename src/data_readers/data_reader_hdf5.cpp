...
void hdf5_reader::read_hdf5_sample(int data_id, conduit::Node& sample) {
  ...
  if (h_data < 0) {
    throw lbann_exception(std::string{} + __FILE__ + " " 
      ...
      + " hdf5_reader::load() - can't find hdf5 key : " 
      + HDF5_KEY_DATA);
    }
...
void hdf5_reader::load() {
  ...
  if (args.get<bool>("disable_mpio_hdf5")) {
  ...
  if(args.get<bool>("disable_mpio_hdf5")) { 
    ...
    if(args.get<bool>("coll_metadata")) {
      H5Pset_all_coll_metadata_ops(m_fapl, true);
    }
    //TODO: should this have the ability to set the size of the buf
    if(args.get<bool>("sieve_buf")) {
      H5Pset_sieve_buf_size(m_fapl, 52262144);  
    }
    //The parallel dataset seems to clash with hyperslab
